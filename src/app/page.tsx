...
const foodCards = [
  ...
  },
  {
    ...
    tip: "2日目の夜におすすめ。フライドとヤンニョムの半々が無難です。",
  },
];
...
const moreFoods = [
  ...
  ["パスタ", "パスタ", "弘大、漢南、江南のカジュアルレストランで選びやすい選択肢。"],
                ...
                <div className="mt-6 grid gap-3">
                  {["朝", "昼", "夕方", "夜"].map((time, stepIndex) => (
                    <div className="timeline-row" key={`${day.day}-${time}`}>
                      <strong>{time}</strong>
                      <div>
                        <h4>{index === 3 && stepIndex === 3 ? "仁川国際空港へ" : "移動と観光を調整"}</h4>
                        <p>
                          {index === 3 && stepIndex === 3
                            ? "14:45までに空港へ到着。LJ209の搭乗手続きに余裕を持ちます。"
                            : "混雑、天気、営業時間に合わせて順番を微調整できます。"}
                        </p>
                      </div>
                    </div>
                  ))}
      ...
      <section id="departure" className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-12">
        <div className="departure-band">
          <div>
            ...
            <p>
              東大門市場、清渓川、光化門の教保文庫は午前中にまとめます。
              昼食後は空港鉄道で仁川へ向かい、チェックインと保安検査の時間を確保してください。
            </p>
          </div>
